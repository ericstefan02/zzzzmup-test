<template>
  ...
  <div
    class="grid grid-cols-2 border border-neutral-200 rounded-xl shadow-lg overflow-hidden max-h-150 bg-white"
  >
    <img
    ...
    />
    <div class="flex flex-col gap-6 px-10 justify-center">
      <div class="flex items-center gap-2 text-neutral-500 text-sm">
        <Icon name="ion:calendar-clear-outline" size="14" />
        <p>{{ formatedDate }}</p>
      </div>
      <h2 class="text-primary-900 font-bold text-3xl line-clamp-3">
        {{ newsArticle.title }}
      </h2>
